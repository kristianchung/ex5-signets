 ...
 */
export async function lireTout(uid,triSelection=['datemodif','desc']) {
  const dossiers = [];
  /************************************************************** Exercice #5 : question A **************************/
  // Modifier très légèrement la ligne suivante
  const reponse = await firestore.collection(utilRef).doc(uid).collection(dossRef).orderBy(triSelection[0],triSelection[1]).get();
  reponse.forEach(
    doc => {
